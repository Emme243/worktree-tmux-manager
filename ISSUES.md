...
## Milestone 1-C — Onboarding Wizard & Settings

> A guided in-app onboarding flow that collects API keys and project config — no manual TOML editing required. Replaces the current first-run `ProjectSetupScreen`-only flow with a multi-step wizard, and adds a settings screen for post-onboarding changes.

- [x] **M1C-01 · SecretInput widget** — New `modules/widgets/secret_input.py`. A composed widget (Label + `Input(password=True)` + hint Static) for masked API key entry. Constructor: `label`, `placeholder`, `hint` (e.g. "Starts with lin_api_"), `id`. Expose `.value` property. Add a Show/Hide toggle button that flips `input.password`. Follow the `DirectoryInput` pattern. Tests in `tests/unit/widgets/test_secret_input.py`: renders label/input/hint, masked by default, toggle reveals/hides, `.value` returns text, empty hint hides the hint widget.

- [x] **M1C-02 · API key validation helpers** — New `modules/core/validation.py` with async functions that test credentials via lightweight API calls. `validate_linear_key(api_key: str) -> tuple[bool, str]`: creates a temporary `LinearClient`, runs `{ viewer { id name } }`, returns `(True, "Authenticated as {name}")` or `(False, error_msg)`. `validate_linear_team(api_key: str, team_id: str) -> tuple[bool, str]`: queries `team(id:) { name }`, returns `(True, "Team: {name}")` or `(False, "Team not found")`. `validate_github_token(token: str) -> tuple[bool, str]`: GET `https://api.github.com/user` with `httpx` (already a dep), returns `(True, "Authenticated as {login}")` or `(False, error_msg)`. All use 5-second timeouts. Tests in `tests/unit/core/test_validation.py`: mock the underlying clients, test success/auth-error/network-error/timeout paths.

- [x] **M1C-03 · Wizard step framework** — New package `modules/screens/wizard/` with `controller.py` and `base_step.py`. `WizardData` dataclass holds all collected values (`linear_api_key`, `linear_team_id`, `github_token`, `project_path`, `github_repo` — all `None` by default). `WizardStep` enum: `WELCOME`, `LINEAR`, `GITHUB`, `PROJECT`, `SUMMARY`. `WizardController` (plain class): holds ordered step list, current index, and `WizardData`; exposes `current_step`, `next()`, `back()`, `skip()`, `is_first`, `is_last`, `progress` (e.g. "Step 2 of 5"); steps can be dynamically disabled. `WizardStepScreen(ModalScreen[str])`: abstract base for steps; receives controller ref; standard layout with progress indicator, abstract `compose_step_content()`, and bottom bar (Back/Skip/Next buttons — hidden/shown contextually). Dismiss result is `"next"`, `"back"`, `"skip"`, or `"cancel"`. Tests in `tests/unit/screens/wizard/test_controller.py`: step navigation, skip, disable, progress string, boundary checks.

- [x] **M1C-04 · Welcome, Linear & GitHub wizard steps** — Three step screens extending `WizardStepScreen`. **WelcomeStepScreen** (`welcome_step.py`): displays app description and two checkboxes — "Set up Linear integration" and "Set up GitHub integration" (both checked by default). Unchecking disables that step in the controller. No Back (first step), no Skip. **LinearStepScreen** (`linear_step.py`): two `SecretInput` fields for API key and team ID, with hints on where to find them. "Validate" button runs `validate_linear_key()` + `validate_linear_team()` via `@work`, shows inline success/error feedback. Next enabled only after validation passes (or user can Skip). **GithubStepScreen** (`github_step.py`): one `SecretInput` for token with scope hint (`repo, read:user`). "Validate" button runs `validate_github_token()`. Same gating as Linear. Note: `github_repo` is per-project and collected in the Project step, not here. Tests: welcome checkboxes toggle steps, validation success/failure enables/blocks Next, Skip works, data stored in `WizardData`.

- [x] **M1C-05 · Project directory & summary wizard steps** — Two step screens. **ProjectStepScreen** (`project_step.py`): contains `DirectoryInput` for the git repo path (reuse existing widget). If GitHub was configured (`WizardData.github_token` is set), also shows an `Input` for "GitHub repo (owner/repo)". Validation: path exists, is git repo, github_repo matches `owner/repo` format if provided. Not skippable. **SummaryStepScreen** (`summary_step.py`): read-only display of everything collected — project path, Linear status (configured/skipped with authenticated username), GitHub status (configured/skipped). "Finish" button builds `AppConfig` from `WizardData` and calls `save_config()` once. Also add `github_repo: str | None = None` to `ProjectConfig` dataclass, update `load_config`/`save_config` to persist it within `[[projects]]` entries, and migrate the top-level `github_repo` into the first project on load. Tests: project validation, github_repo field shown conditionally, summary displays correct data, config round-trips with per-project github_repo.

- [x] **M1C-06 · Wire wizard into app startup** — Update `modules/app.py`: when `ConfigError` is caught with `reason in ("missing_file", "missing_repo_path")`, launch the wizard instead of `ProjectSetupScreen`. New `_run_wizard()` method creates `WizardController`, pushes the first step, and on each dismiss result (`"next"/"skip"/"back"/"cancel"/"finish"`) advances/retreats/exits accordingly. On `"finish"`: reload config, proceed to `WorktreeListScreen`. On `"cancel"`: exit app. Keep `ProjectSetupScreen` for the "add project" flow from `ProjectPickerScreen` (it doesn't re-ask for API keys since those are global). Tests: missing config triggers wizard, existing config skips wizard, wizard completion leads to worktree list, cancel exits app.

- [x] **M1C-07 · Settings screen** — New `modules/screens/settings.py` with `SettingsScreen(ModalScreen[bool])`. Layout: sections for Linear (SecretInput for API key, Input for team ID, Validate button) and GitHub (SecretInput for token, Validate button), pre-populated from current `AppConfig`. "Save" validates changed fields, updates config, calls `save_config()`, dismisses with `True`. "Cancel" dismisses with `False`. Add `Binding("S", "settings", "Settings")` to `WorktreeListScreen` — on dismiss with `True`, reload config. Note: `github_repo` is per-project (not edited here — future project-edit modal). Tests: renders with pre-populated values, save updates config, cancel doesn't save, validation feedback shows inline.
